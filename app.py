...
def tela_login():
    ...
    st.subheader("Faca login ou crie sua conta para acessar o sistema")
...
if not st.session_state["autenticado"]:
    ...
    if st.session_state['cargo_atual'] in ["Administrador", "Funcionário 2B"]:
        ...
        st.sidebar.success("CONTA VERIFICADA")
    else:
        st.sidebar.markdown(f"**Logado como:** {st.session_state['usuario_atual']}")
        st.sidebar.markdown(f"**Cargo:** {st.session_state['cargo_atual']}")
        st.sidebar.warning("AGUARDANDO LIBERAÇÃO")
    ...
    if menu == "Confirmacao de Agendamento":
        st.title("Sistema de Confirmacao de Agendamento")
        
        if not verificar_permissao():
            st.error("Acesso Pendente: Sua conta esta aguardando liberacao do Administrador.")
        else:
            st.subheader("Preencha os dados abaixo para gerar o PDF oficial")
        ...
        st.title("Atualizador de PDF Cadastral")
        
        if not verificar_permissao():
            st.error("Acesso Pendente: Aguardando liberacao do Administrador.")
        else:
            st.markdown("Cole a ficha do cliente abaixo para extrair os dados e gerar o PDF")
        ...
        st.title("Gerador de Boletim Web (BOU)")
        
        if not verificar_permissao():
            st.error("Acesso Pendente: Aguardando liberacao do Administrador.")
        else:
            st.subheader("Preencha os dados abaixo para gerar e baixar o PDF oficial do Boletim")
        ...
        st.title("Sistema de Alvaras - Tropa do Adv")
        
        if not verificar_permissao():
            st.error("Acesso Pendente: Aguardando liberacao do Administrador.")
        else:
            st.subheader("Cole o texto do alvará abaixo para extrair os dados e gerar o PDF automaticamente")
        ...
        st.title("Painel de Gerenciamento de Contas")
        st.subheader("Mude o cargo dos usuários para liberar o acesso deles")
        ...
        st.title("Auditoria de Acessos em Tempo Real")
        st.subheader("Acompanhe quem acessou o sistema, data/hora e o sistema operacional")
